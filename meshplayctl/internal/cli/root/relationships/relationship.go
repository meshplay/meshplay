...
var RelationshipCmd = &cobra.Command{
	...
	Long:  "Meshplay uses relationships to define how interconnected components interact. View list of relationships and detailed information of a specific relationship",
	Example: `
...
meshplayctl exp relationships search --[flag] [query-text]
	...
	Args: func(cmd *cobra.Command, args []string) error {
		if len(args) == 0 {
			errMsg := "Usage: meshplayctl exp relationships [subcommand]\nRun 'meshplayctl exp relationships --help' to see detailed help message"
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		if ok := utils.IsValidSubcommand(availableSubcommands, args[0]); !ok {
			return errors.New(utils.RelationshipsError(fmt.Sprintf("'%s' is an invalid subcommand. Please provide required options from [view]. Use 'meshplayctl exp relationships --help' to display usage guide.\n", args[0]), "relationship"))
		}
