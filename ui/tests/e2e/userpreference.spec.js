...
const userPreferenceTests = [
  ...
  },
  {
    ...
    apiURL: `${ENV.MESHPLAY_SERVER_URL}/api/user/prefs?contexts=all`,
...
test.describe('User Preferences Page Tests', () => {
  test.beforeEach(async ({ page }) => {
    const userPrefReq = page.waitForRequest(
      (request) =>
        request.url().startsWith(`${ENV.MESHPLAY_SERVER_URL}/api/user/prefs`) &&
        request.method() === 'GET',
    );
    const userPrefRes = page.waitForResponse(
      (response) =>
        response.url().startsWith(`${ENV.MESHPLAY_SERVER_URL}/api/user/prefs`) &&
        response.status() === 200,
    );

    // Visit User Preferences Page
    await page.goto(`${ENV.MESHPLAY_SERVER_URL}/user/preferences`);
